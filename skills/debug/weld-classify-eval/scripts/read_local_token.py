...
def leveldb_dir(browser: str, profile: str) -> Path:
    base = Path.home() / "Library" / "Application Support" / BROWSER_DIRS[browser]
...
def aes_ecb_decrypt(raw: bytes) -> bytes | None:
    if _HAVE_PYCRYPTO:
        ...
        proc = subprocess.run(
            ["openssl", "enc", "-d", "-aes-128-ecb", "-K", AES_KEY.hex(), "-nopad"],
            input=raw,
            capture_output=True,
        )
...
def main() -> None:
    ...
    parser.add_argument("--browser", default="chrome", choices=sorted(BROWSER_DIRS))
    ...
    hits = scan(leveldb_dir(args.browser, args.profile), args.origin)
    if not hits:
        raise SystemExit(
            "没解出 token。确认：1) 浏览器里已登录 designorder；2) --browser/--profile 对；"
            "3) --origin 是否过滤太严（试 --origin ''）"
        )
